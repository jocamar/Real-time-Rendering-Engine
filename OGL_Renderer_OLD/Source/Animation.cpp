...
LinearAnimation::LinearAnimation(vector<glm::vec3> ctrlPoints, float span, bool looping) : Animation(ctrlPoints)
{
	...
	if (ctrlPoints.size() > 0)
	{
		...
		//Calculate initial direction and orientation from first control point
...
void LinearAnimation::update(unsigned long milis, bool animationsPaused)
{
	...
	if (curPoint < ctrlPoints.size())
	{
		...
		//Calculate distance to move in the 3 axes since last update
		...
		//If the object would move more than the distance to the following control point
		if (distToMove > distToNextCtrlPoint)
		{
			//Keep moving through the control points to find the next one
			while (distToMove > distToNextCtrlPoint) {
				...
				{
					//If we got to the end and we're looping reset animation
					if (looping && ctrlPoints.size() > 0)
					{
						reset();
						finished = false;
					}
					//otherwise set the animation as finished
					else
					{
						curPos = ctrlPoints[curPoint];
						finished = true;
			...
			//Move the object the remaining distance
		...
		//If the object would not reach the next control point just move it in the same direction by the calculated distance
		else
		{
			curPos.x = curPos.x + curDirection.x*distToMove;
			curPos.y = curPos.y + curDirection.y*distToMove;
