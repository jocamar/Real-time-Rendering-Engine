...
void SceneNode::update(float millis) {
	//If node has an animation update it and the node's position and orientation
	if (animation!=nullptr)
	{ 
	...
	//Generate translation, rotation and scale matrixes
	auto m_pos = glm::translate(glm::mat4(), position);
	auto m_scale = glm::scale(glm::mat4(), scale);
	auto m_rot = glm::mat4_cast(rot);
	orientation = rot;

	//Generate matrixes move, scale and rotate from correct origin
	...
	//If node has a parent apply its model matrix too
	if(this->parent)
...
RenderOrder SceneNode::getRenderEntities(int material, Camera *camera, bool shadowMap, Globals::LIGHT_TYPE shadowType)
{
	...
	for(auto child : children)
	{
		//If node has material use it, otherwise inherit from parent
		int materialToUse;
		if (this->material >= 0)
		...
		//Get render entities from child
