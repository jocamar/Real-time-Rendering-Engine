...
Mesh::Mesh(const char *id, SceneManager *manager, vector<Vertex> vertices, vector<GLuint> indices, BoundingBox boundingBox, const char *materialId)
{
	...
	//setup the buffers from the provided vertices and indices
	setupMesh();
}
...
void Mesh::setupMesh()
{
	//Generate Array Object, Buffer Object and Element Array Object
...
Model::Model(const char *id, SceneManager *manager, const char *path)
{
	...
	if(path)
	{
		//Import model from file using assimp
		...
		//Process all the assim scene node, starting with the root
...
Mesh* Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	//Minimum and maximum values to compute bounding box
	float max_x = FLT_MIN;
	float max_y = FLT_MIN;
	float max_z = FLT_MIN;
	...
	//Manually calculate tangents and bitangents (not necessary if assimp flag is set to do it for us)
	//Comment out if assimp is set to calculate these
	...
	for (GLuint i = 0; i < mesh->mNumVertices; i++)
	{
		...
		//Update maximum and minimum values according to vertex position
		if(vector.x > max_x)
		{
			max_x = vector.x;
		}
		...
		if (mesh->mTextureCoords[0])
		{
			...
			//Interpolate previously calculated tangents and bitangents and set them up
			//Comment out if assim is set to calculate these already
			/*glm::vec3 tangent(0);
		...
		//Push back processed vertex
		vertices.push_back(vertex);
	}

	//Calculate bounding box from previously computed minimum and maximum values
	...
	if (mesh->mMaterialIndex >= 0)
	{
		...
		//Create an automatic ID for the material based on the ID of the model
		...
		//Get material shading parameters
...
vector<Texture*> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, char *typeName)
{
	...
	for (GLuint i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		{   // If texture hasn't been loaded already, load it
			...
			//Generate an automatic texture ID from the texture name
			...
			//Add it to material textures
			textures.push_back(texture);
			// Register it with the manager, after this it won't need to be loaded again since the ID is already present
			this->manager->addTexture(texture);  
		}
		else
		{
			//Add texture to material textures
...
{
	//Generate texture ID and load texture data using SOIL
