					...
					GLfloat constant, GLfloat linear, GLfloat quadratic, const char *modelId, SceneNode* parent) : Entity(idLight, manager, modelId, parent, false)
{
	...
	//Generate the FBO and textures for the shadow maps
	...
	//We're creating an omni-directional light so we need a cubemap for the shadow maps
	...
	//We only need to attach the depthbuffer so we set the draw and read to none
	glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
	glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, depthCubemap, 0);
	glDrawBuffer(GL_NONE);
	glReadBuffer(GL_NONE);

	//Check to see if framebuffer is correct
...
{
	//Bind the framebuffer and clear the depth buffer before rendering
	glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
	glClear(GL_DEPTH_BUFFER_BIT);

	//Create correct cameras depending on what kind of shadows we're generating
	if (this->directional)
	{
	...
	if (!cam->Ortho)
	{
		//If we're generating omni-directional shadow maps create the 6 viewproj matrixes, add them to the camera and render the scene
	...
	{
		//if we're generating directional shadow maps create the viewproj matrix add it to the camera and render the scene
		this->lightSpaceMatrix = projection * view;
		cam->ViewProjMatrix = lightSpaceMatrix;
		manager->render(cam, true);
	}

	//reset the framebuffer and the matrixes and camera
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
